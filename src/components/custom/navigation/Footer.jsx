...
import { newsletterService } from '../../../lib/firebaseServices';
...
const Footer = () => {
  ...
  const handleNewsletterSubmit = async (e) => {
    ...
    try {
      const result = await newsletterService.create({ email: email.trim() });
      
      if (result.success) {
        setMessage('Thank you for subscribing! You\'ll receive updates on Rod\'s community work.');
        setEmail('');
      } else {
          ...
          <div>
            ...
            <form onSubmit={handleNewsletterSubmit} className="space-y-3">
              <input 
                ...
                className="w-full px-4 py-2 bg-gray-800 text-white border border-gray-700 focus:outline-none focus:ring-2 focus:ring-red-500 disabled:opacity-50"
                required
              ...
              <button 
                type="submit"
                disabled={isSubmitting}
                className="w-full bg-red-600 hover:bg-red-700 disabled:bg-red-400 text-white font-bold py-2 px-4 transition-colors duration-300 disabled:cursor-not-allowed"
              >
                {isSubmitting ? (
